...
export function SeoHead({
    ...
    const description =
        t(descKey) === descKey ? t('meta.description') : t(descKey);
    ...
    return (
        <Head>
            {/*
              head-key values MUST match the `inertia="..."` attributes in
              resources/views/partials/seo.blade.php so Inertia replaces the
              server-rendered tags in place on hydration instead of duplicating
              them. <title> is special-cased by Inertia and needs no key.
            */}
            <title>{title}</title>
            <meta
                ...
                content={description}
            />
            <meta head-key="theme-color" name="theme-color" content="#6f209d" />
